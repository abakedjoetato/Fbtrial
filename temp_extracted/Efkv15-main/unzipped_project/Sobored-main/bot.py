...
# Get version safely to handle cases where __version__ isn't available
try:
    from discord import __version__ as discord_version
except ImportError:
    discord_version = "unknown (mock implementation)"
    
from utils.command_imports import is_compatible_with_pycord_261
logger.info(f"Using discord library version: {discord_version}")
...
class Bot(commands.Bot):
    ...
    def __init__(self, *, production: bool = False, debug_guilds: Optional[List[int]] = None):
        ...
        # Use direct prefix "!" instead of when_mentioned_or for compatibility
        super().__init__(
            command_prefix="!",  # Simple prefix to avoid dependency on when_mentioned_or
            intents=intents,
