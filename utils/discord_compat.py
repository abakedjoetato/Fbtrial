...
# Define our own versions of these classes to avoid import errors
# with py-cord vs discord.py compatibility
class SlashCommandGroup:
    """Compatibility class for SlashCommandGroup across discord libraries"""
    ...
    def command(self, name=None, description=None, **kwargs):
        """Register a command on this group"""
        def decorator(func):
...
class ApplicationContext:
    """Compatibility class for ApplicationContext across discord libraries"""
    pass

class Interaction:
    """Compatibility class for Interaction across discord libraries"""
    pass
...
def get_channel_from_context(ctx: ApplicationContext) -> Optional[Any]:
    """
    ...
    try:
        ...
        if guild and hasattr(ctx, "channel_id") and ctx.channel_id is not None:
            # Using bot.get_channel instead as a safer alternative
            if hasattr(ctx, "bot") and ctx.bot is not None:
