...
def command_handler(
    ...
    Args:
        ...
        admin_only: Whether the command requires administrator permissions
    ...
    def decorator(func):
        ...
        async def wrapped(self, ctx, *args, **kwargs):
            ...
            try:
                # Check if admin_only is set and enforce it
                if admin_only:
                    # Try to get the member's permissions
                    has_admin_perms = False
                    
                    try:
                        # For interactions (slash commands)
                        if hasattr(ctx, 'permissions') and ctx.permissions:
                            has_admin_perms = ctx.permissions.administrator
                        # For older context objects
                        elif hasattr(ctx, 'author') and hasattr(ctx.author, 'guild_permissions'):
                            has_admin_perms = ctx.author.guild_permissions.administrator
                        # For member context in interactions
                        elif hasattr(ctx, 'user') and hasattr(ctx, 'guild') and ctx.guild:
                    ...
                    # If user doesn't have admin permissions, send error and return
