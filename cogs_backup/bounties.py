...
class Bounties(commands.Cog):
    ...
    # Use more compatible approach with commands.slash_command
    ...
    async def bounty(self, ctx):
        ...
        # Just a group command - doesn't do anything by itself
        pass
    ...
    @option("player_name", str, description="Name of the player to place a bounty on")
    @option("amount", int, description="Amount of currency to offer as a bounty")
    @option("server_id", str, description="The server ID (optional)", required=False)
    ...
    @command_handler(guild_only_command=True)
