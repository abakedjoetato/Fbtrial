...
# Import discord directly for namespace resolution, but don't use it directly
import discord

from discord_compat_layer import (
    Embed, Color, commands, Interaction, app_commands, 
    slash_command, SlashCommandGroup, Option, Member, Guild, User
)
...
from models.guild import Guild as GuildModel
...
class Economy(commands.Cog):
    ...
    @app_commands.autocomplete(server_id=server_id_autocomplete)
    @premium_feature_required(feature_name="economy", min_tier=2)  # Economy requires premium tier 2+
    async def give(self, ctx, server_id: str, user: Member, amount: int):
    ...
    @premium_feature_required(feature_name="economy", min_tier=2)  # Economy requires premium tier 2+
    async def adjust_credits(self, ctx, server_id: str, user: Member, amount: int, reason: str = "Admin adjustment"):
    ...
    @app_commands.autocomplete(server_id=server_id_autocomplete)
    @premium_feature_required(feature_name="economy", min_tier=2)  # Economy requires premium tier 2+
    async def transactions(self, ctx, server_id: str, user: Optional[Member] = None, limit: int = 10):
