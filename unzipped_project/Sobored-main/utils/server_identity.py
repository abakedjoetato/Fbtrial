...
class ServerIdentity:
    """
    Represents a server's identity with various identifiers.
    
    This class provides a consistent way to handle different identifiers 
    for a server (UUID, numeric ID, hostname, etc.) and relationships
    between them.
    """
    ...
    def __init__(self, 
        ...
        """
        Initialize a ServerIdentity instance.
        
        Args:
            server_id: The UUID or primary identifier of the server
            numeric_id: The numeric ID of the server
            hostname: The server hostname
            server_name: The friendly name of the server
            guild_id: The Discord guild ID associated with this server
        """
        self.server_id = str(server_id) if server_id else None
        self.numeric_id = str(numeric_id) if numeric_id else None
        self.hostname = hostname
        self.server_name = server_name
        self.guild_id = str(guild_id) if guild_id else None
        
        # If numeric_id not provided, derive it
        if self.numeric_id is None and self.server_id:
            self.numeric_id, _ = identify_server(
                self.server_id, 
                self.hostname,
                self.server_name,
                self.guild_id
    ...
    def path_components(self) -> Tuple[str, str]:
        """
        Get the path components for this server.
        
        Returns:
            Tuple of (server_dir, path_server_id)
        """
        return get_path_components(
            self.server_id,
            self.hostname or "",
            self.numeric_id,
            self.guild_id
    ...
    def server_dir(self) -> str:
        ...
        """
        dir_name, _ = self.path_components
        return dir_name
        
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'ServerIdentity':
        """
        Create a ServerIdentity from a dictionary.
        
        Args:
            data: Dictionary containing server data
            
        Returns:
            A new ServerIdentity instance
        """
        return cls(
            server_id=data.get('_id') or data.get('server_id'),
            numeric_id=data.get('numeric_id'),
            hostname=data.get('hostname'),
            server_name=data.get('name') or data.get('server_name'),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary representation of this ServerIdentity
        """
        return {
